...
sql = "SELECT L.STOCK_NAME,L.STOCK_SYMBOL,COUNT(DISTINCT L.USERNAME) AS con FROM (SELECT * FROM fairy.t_changelist t WHERE t.TARGET_WEIGHT > t.PREV_WEIGHT_AJJUSTED AND t.updated_at > '2017-01-24' AND t.sign in  ('WEEK','MONTH')) L GROUP BY L.STOCK_NAME,L.STOCK_SYMBOL ORDER BY con DESC"
print sql
try:
   # 执行SQL语句
   cursor.execute(sql)
